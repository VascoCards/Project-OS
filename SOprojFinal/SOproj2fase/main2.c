...
MUTATION_ARRAY * mutationArray[GENOMAS];

MUTATION_ARRAY *produz();

void consome(MUTATION_ARRAY *pArray);
...
int main() {
    ...
    //threads
    pthread_t threadsCons[NUM_THREADS];
    pthread_t threadsProd[NUM_THREADS];
    int rc, i;

    sem_init(&pode_prod, 0, GENOMAS);
    ...
    for( i=0; i < NUM_THREADS; i++ )
    {
        ...
        if (pthread_create(&threadsProd[i], NULL, &Producer, (GENOME *)g) != 0) {
            perror("Failed to create thread");
        }
    }

    for( i=0; i < NUM_THREADS; i++ ) {
        if (pthread_create(&threadsCons[i], NULL, &Consumer, NULL) != 0) {
            perror("Failed to create thread");
        }
    }
    ...
    for( i=0; i < 10; i++ ) {
        ...
        if (pthread_join(threadsProd[i], NULL) != 0) {
            perror("Failed to join thread");
        }
        if (pthread_join(threadsCons[i], NULL) != 0) {
            perror("Failed to join thread");
        }
    }
    //pthread_exit(NULL);
...
void *Consumer(){
    ...
    while (1){
        ...
        MUTATION_ARRAY * aux = mutationArray[consptr];

        consptr = (consptr+1) % GENOMAS;
        sem_post(&pode_prod);
        pthread_mutex_unlock(&mutexC);

        Consome(aux);
        if(genomasProc == GENOMAS)
            pthread_exit(NULL);
    }
}

void Consome(MUTATION_ARRAY *pArray) {
    save_mutation_array(pArray, "Mutacoes.txt", 0, pthread_self());
    printf("Consumiu");
}


void * Producer(void *threadarg){
    GENOME * aux = threadarg;

    while(1){
        MUTATION_ARRAY *arrayAux = produz(aux);
        sem_wait(&pode_prod);
        pthread_mutex_lock(&mutexP);

        mutationArray[prodptr] = arrayAux;
        pthread_mutex_lock(&mutex);
        genomasProc++;
        pthread_mutex_unlock(&mutex);

        prodptr = (prodptr+1) % GENOMAS;
...
{
    float perc = 100.0 * genomasProc / GENOMAS;
    printf("%d Concluido\n", (int) perc);
    fflush(stdin);
    //signal(SIGSEGV,sig_func);
}
 ...
 */
void save_mutation_array (MUTATION_ARRAY * mutation_array, char * path, int detail, long id) {
    ...
    for (int i=0 ; i<mutation_array->n_mutations ; i++) {
        ...
        sprintf(buf, "%ld|%s;%s|%s|(%d)", id, aux->genome_a, aux->genome_b, aux->gene, aux->seq_mutations.n);
        if (detail) {
