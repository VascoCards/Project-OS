...
int main(int argc, char *argv[ ]) {
    ...
    for (int i = 0; i < Nfilhos; i++) {
        ...
        if (pids[i] == 0) {
            ...
            while (g != NULL) {
                ...
                kill(getpid(), SIGUSR1);
                send_pipe_mutation_array (mutation_array, fds, 0);
                GenomesProc++;
    ...
    long n;
    while ((n = readn(fds[0], parts, sizeof(parts))) > 0) {
        writen(fd, parts, n);
    }
    close(fds[0]);
    close(fd);
...
void send_pipe_mutation_array (MUTATION_ARRAY * mutation_array, int * fds, int detail) {
